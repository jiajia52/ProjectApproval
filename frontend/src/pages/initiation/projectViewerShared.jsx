...
function buildProjectValueEntries(documentPayload, activeTab) {
  ...
  const groups = [
    ...
    { key: "management", label: "管理体系模型", items: normalizeList(tamModels.management) },
  ];
  ...
  return [
    {
      ...
      groupLabel: activeTab === "result" ? "结果（财务/客户）模型" : activeTab === "management" ? "管理体系模型" : "能力（竞争力）模型",
      title: "项目价值说明",
      benefitDepartment: "-",
...
const SPECIAL_HISTORY_INVESTMENT_BY_PROJECT_CODE = {
  "BA-2601E": {
    previousProjects: ["基础软件-WPS终端项目"],
    historyTotalCost: 2081600,
    rows: [
      {
        index: 1,
        project_year: "2024",
        project_name: "基础软件-WPS终端项目",
        project_content: "",
        project_amount: 2081600,
        related_cost: 2081600,
      },
    ],
  },
};

function hasMeaningfulHistoryRow(row) {
  if (!row || typeof row !== "object") {
    return false;
  }
  const year = String(firstMeaningful(row.project_year, row.year, row.projectYear, "")).trim();
  const name = String(firstMeaningful(row.project_name, row.projectName, "")).trim();
  const amount = parseNumeric(firstMeaningful(row.project_amount, row.projectAmount, row.amount, ""));
  const relatedCost = parseNumeric(firstMeaningful(row.related_cost, row.relatedCost, row.involveFee, ""));
  return Boolean((year || name) && (Number.isFinite(amount) || Number.isFinite(relatedCost)));
}

function buildPatchedHistoryInvestmentData(documentPayload) {
  const costChange = documentPayload?.cost_change || {};
  const projectCode = String(
    firstMeaningful(
      documentPayload?.project_summary?.project_code,
      documentPayload?.project_summary?.projectCode,
      documentPayload?.project_summary?.serialNo,
      documentPayload?.project_code,
      documentPayload?.projectCode,
      "",
    ),
  ).trim().toUpperCase();
  const directRows = normalizeList(costChange.history_items)
    ...
    .filter(hasMeaningfulHistoryRow);

  const specialFallback = SPECIAL_HISTORY_INVESTMENT_BY_PROJECT_CODE[projectCode] || null;
  const useSpecialFallback = !directRows.length && Boolean(specialFallback);
  const rows = useSpecialFallback ? normalizeList(specialFallback?.rows) : directRows;
  const previousProjects = normalizeList(costChange.previous_projects).length
    ? normalizeList(costChange.previous_projects)
    : useSpecialFallback
      ? normalizeList(specialFallback?.previousProjects)
      : Array.from(new Set(rows.map((row) => String(row.project_name || "").trim()).filter(Boolean)));
  ...
  const historyTotalCost = useSpecialFallback
    ? firstMeaningful(
      specialFallback?.historyTotalCost,
      Number.isFinite(inferredTotal) && inferredTotal > 0 ? inferredTotal : "",
      costChange.history_total_cost,
    )
    : firstMeaningful(
  ...
  const hasHistoryCost = typeof costChange.has_history_cost === "boolean"
    ? costChange.has_history_cost
    : rows.length > 0 || useSpecialFallback;

  return {
    rows,
    hasHistoryCost: useSpecialFallback ? true : hasHistoryCost,
    previousProjects,
    historyTotalCost,
    historyAnalysis: costChange.history_analysis || "",
  };
...
function buildCostChangeMetric(documentPayload) {
  const costChange = documentPayload?.cost_change || {};
  const primaryItem = firstMeaningful(
    normalizeList(costChange.change_items)[0],
    normalizeList(documentPayload?.remote_snapshot?.endpoints?.cost_change?.data)[0],
    {},
  );
  ...
  return {
    dimension: firstMeaningful(primaryItem.dimension, primaryItem.dimensionalityName, primaryItem.dimensionality, "-"),
    firstLabel: firstMeaningful(primaryItem.first_year_label, primaryItem.oneKey, "2024年目标"),
    firstValue: firstMeaningful(primaryItem.first_year_value, primaryItem.oneValue, ""),
    secondLabel: firstMeaningful(primaryItem.second_year_label, primaryItem.twoKey, "2025年目标"),
    secondValue: firstMeaningful(primaryItem.second_year_value, primaryItem.twoValue, ""),
    thirdLabel: firstMeaningful(primaryItem.third_year_label, primaryItem.threeKey, "2026年目标"),
    thirdValue: firstMeaningful(primaryItem.third_year_value, primaryItem.threeValue, ""),
    secondThirdTrend: firstMeaningful(primaryItem.second_third_value, primaryItem.twoThreeValue, ""),
    content: firstMeaningful(primaryItem.content, costChange.reason, ""),
  };
}

function PatchedHistoryInvestmentPanel({ documentPayload }) {
  const data = buildPatchedHistoryInvestmentData(documentPayload);
  return (
    <div className="history-investment-panel">
      <div className="history-investment-form-row">
        <div className="history-investment-field">
...
export {
  ...
  sectionDefinitions,
  tabDefinitions,
  changeProjectSectionDefinitions,
  changeProjectTabDefinitions,
  systemProjectSectionDefinitions,
  systemProjectTabDefinitions,
  EXCEL_PROJECT_CONTENT_TABS_STANDARD,
  EXCEL_PROJECT_CONTENT_TABS_SYSTEM,
  EXCEL_LAYOUTS,
  EXCEL_SYSTEM_CONSTRUCTION_SUBCATEGORIES,
  EXCEL_OPERATION_SERVICE_SUBCATEGORIES,
  EXCEL_TECHNICAL_SERVICE_SUBCATEGORIES,
  EXCEL_VALUE_ONLY_SUBCATEGORIES,
  EXCEL_INFRASTRUCTURE_REVIEW_SUBCATEGORIES,
  EXCEL_INFRASTRUCTURE_NO_CHANGE_SUBCATEGORIES,
  LATEST_EXCEL_ARCHITECTURE_REVIEW_KEYS_ALL,
  LATEST_EXCEL_ARCHITECTURE_REVIEW_KEYS_DATA_AND_SECURITY,
  LATEST_EXCEL_ARCHITECTURE_REVIEW_KEYS_SECURITY_ONLY,
  LATEST_EXCEL_LAYOUTS,
  LATEST_EXCEL_LAYOUT_BY_SUBCATEGORY,
  NORMALIZED_LATEST_LAYOUT_BY_SUBCATEGORY,
  LAYOUT_ALIAS_TO_SUBCATEGORY,
  SPECIAL_HISTORY_INVESTMENT_BY_PROJECT_CODE,
  ARCHITECTURE_REVIEW_PREVIEW,
  createLatestExcelLayout,
  normalizeLayoutKey,
  normalizeList,
  buildImageUrl,
  formatValue,
  ...
  buildSystemProjectScopeEntries,
  buildProjectValueEntries,
  normalizeDepartmentValue,
  projectValueGroupFromType,
  projectValueGroupLabel,
  buildOkrEntries,
  buildProjectValueEntriesById,
  buildTamModelEntries,
  buildSystemScopeRows,
  buildSystemScopeTree,
  budgetMetricValue,
  buildBudgetSummary,
  buildBudgetRows,
  procurementMethodLabel,
  buildProjectBadges,
  isChangeProjectSummary,
  isSystemDevelopmentSummary,
  resolveExcelLayout,
  hasProjectValueData,
  ...
  isPassDecision,
  approvalToneClass,
  buildApprovalHighlights,
  buildProjectCommentary,
  ...
  BudgetTable,
  parseNumeric,
  buildHistoryInvestmentData,
  HistoryInvestmentPanel,
  hasMeaningfulHistoryRow,
  buildPatchedHistoryInvestmentData,
  ...
  renderArchitectureSummaryCards,
  ArchitectureReviewDetailDialog,
  ArchitectureReviewPanel,
  ArchitectureReviewBoard,
  inferOrganizationPartyType,
  buildOrganizationRows,
  buildThirdPartySummary,
  buildOrganizationChartRows,
  OrganizationBoard,
